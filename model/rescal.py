...
class RescalScorer(RelationalScorer):
    ...
    def score_emb(
        ...
        if combine == "spo":
            ...
            ).sum(dim=-1)  # and sum to obtain predictions
        elif combine == "sp_":
            out = (s_emb.unsqueeze(1).bmm(p_mixmat).view(
                batch_size, entity_size).mm(o_emb.transpose(0, 1)))
        elif combine == "_po":
            out = (p_mixmat.bmm(o_emb.unsqueeze(2)).view(
                batch_size, entity_size).mm(s_emb.transpose(0, 1)))
        else:
...
def rescal_set_relation_embedder_dim(config, dataset, rel_emb_conf_key):
    ...
    if dim < 0:  # autodetect relation embedding dimensionality
        ent_emb_conf_key = rel_emb_conf_key.replace("relation_embedder",
                                                    "entity_embedder")
